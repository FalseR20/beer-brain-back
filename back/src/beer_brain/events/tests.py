...
from .models import Event, Deposit, Repayment
...
class DepositTests(TestCase):
    ...
    def test_delete_deposit(self):
        ...
        response = self.client.delete(url)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertFalse(Deposit.objects.filter(id=deposit.id).exists())
...
class RepaymentTests(TestCase):
    ...
    def test_delete_repayment(self):
        ...
        response = self.client.delete(url)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertFalse(Repayment.objects.filter(id=repayment.id).exists())
...
class DetailedEventTests(TestCase):
    ...
    def create_deposit(self, value=100.00):
        ...
        return Deposit.objects.create(**deposit_data)
    ...
    def create_repayment(self, value=50.00):
        ...
        return Repayment.objects.create(**repayment_data)
