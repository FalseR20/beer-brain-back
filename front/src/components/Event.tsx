...
function EventChat() {
  ...
  return (<>
    <ListGroup variant={"flush"} className={"p-0"}>
      {chatElements.map(chatElement => (<ListGroup.Item className={"d-flex flex-row border-0"} key={chatElement.datetime}>
        <Avatar member={chatElement.member} />
...
function Avatar({ member }: { member: { id: number } }) {
  return <div
...
function Payers({ event }: { event: IEvent }) {
  const membersWithDeposits = event.members.filter((member) => member.deposits.length > 0);
  return (<>
    ...
    <div className={"border border-secondary rounded-3 p-2"}>
      <ListGroup className={""} variant={"flush"}>
        {membersWithDeposits.length > 0 ? (event.members.map((member) => member.deposits.map((deposit) => (
